...
import { Loader2, FileCode2, FilePlus2, FileX2, FileEdit, ChevronRight, ChevronDown, Folder, ArrowLeft, Eye, RefreshCw } from "lucide-react";
import * as api from "../lib/api";

interface LiveDiffViewProps { sessionId: string }
...
function FileViewer({ sessionId, filePath, fileType, onBack }: { sessionId: string; filePath: string; fileType: FileChange["type"]; onBack: () => void }) {
  const [content, setContent] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);
  useEffect(() => { setLoading(true); setError(null); setContent(null); api.getSessionFile(sessionId, filePath).then((r) => setContent(r.content)).catch((e) => setError(e.message)).finally(() => setLoading(false)); }, [sessionId, filePath]);
...
export function LiveDiffView({ sessionId }: LiveDiffViewProps) {
  ...
  useEffect(() => {
    const interval = setInterval(() => fetchDiff(true), 30_000);
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2.5 px-3 py-1.5 bg-[var(--color-surface-raised)] border-b border-[var(--color-border)] min-h-[36px] shrink-0">
        ...
        <span className="text-xs font-semibold truncate min-w-0">Live Changes</span>
      ...
      {diff !== null && !loading && files.length > 0 && (
        <div className="flex flex-1 overflow-hidden">
          ...
          {viewingFile ? (
            <FileViewer sessionId={sessionId} filePath={viewingFile} fileType={viewingFileType} onBack={() => setViewingFile(null)} />
          ) : (
            <div className="flex-1 overflow-auto min-w-0">
              {sections.map((sec) => (
