...
describe("getPortForwardUrl", () => {
  it("returns the proxyUrl for a normal http URL", () => {
    const pf = makePf({ proxyUrl: "http://p-3000-abc.localhost:7700" });

    expect(getPortForwardUrl(pf)).toBe("http://p-3000-abc.localhost:7700");
  });
  ...
  it("falls back to localhost when proxyUrl is empty", () => {
    ...
    expect(getPortForwardUrl(pf)).toBe("http://localhost:19005");
  });
